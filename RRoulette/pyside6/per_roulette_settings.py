...
PER_ROULETTE_KEYS: frozenset = frozenset({
    ...
    "spin_preset_profile",
    "spin_preset_random",
    "spin_duration_random",
    "spin_duration_random_ratio",
    "spin_phase_randomize",
    "spin_phase_overrides",
    "spin_mode",
    "double_duration",
    "triple_duration",
    "sound_tick_enabled",
    ...
    "tick_custom_file",     # v0.6.1: カスタムスピン音ファイル（per-roulette）
    "win_custom_file",      # v0.6.1: カスタム決定音ファイル（per-roulette）
    "profile_idx",          # v0.6.1: ルーレットサイズ（per-roulette）
    "spin_direction",
    "donut_hole",
    "pointer_angle",
    "text_size_mode",
...
class PerRouletteSettings:
    ...
    spin_preset_profile: str = 'z'        # 'x'=AIお勧め / 'y'=緩急強調 / 'z'=バランス型
    spin_preset_random: bool = False       # 毎スピンでプロファイルをランダム抽選
    spin_duration_random: bool = False     # 毎スピンで秒数をランダム抽選
    spin_duration_random_ratio: float = 0.0  # 終了時間ランダム化割合 (0-0.5)
    spin_phase_randomize: float = 0.0     # スピン詳細ランダム化強度 (0-1)
    spin_phase_overrides: dict | None = None  # None = プロファイルデフォルト
    spin_mode: int = 0
    double_duration: float = 5.0
    triple_duration: float = 5.0

    # サウンド ON/OFF（音量はアプリ共通 → AppSettings 側）
    sound_tick_enabled: bool = True
    sound_result_enabled: bool = True
    # i370: 音の種類（per-roulette）
    tick_pattern: int = 0
    win_pattern: int = 0
    # v0.6.1: カスタム音ファイル（per-roulette）
    tick_custom_file: str = ""
    win_custom_file: str = ""

    # v0.6.1: ルーレットサイズ（per-roulette）
    profile_idx: int = 0
    ...
    # 特殊演出
    spin_effects: SpinEffectSettings = field(default_factory=default_spin_effect_settings)
    ...
    def from_app_settings(cls, s: object) -> "PerRouletteSettings":
        ...
        return cls(
            spin_duration=_g("spin_duration", 5.0),
            spin_preset_profile=_g("spin_preset_profile", 'z'),
            spin_preset_random=_g("spin_preset_random", False),
            spin_duration_random=_g("spin_duration_random", False),
            spin_duration_random_ratio=_g("spin_duration_random_ratio", 0.0),
            spin_phase_randomize=_g("spin_phase_randomize", 0.0),
            spin_phase_overrides=None,
            spin_mode=_g("spin_mode", 0),
            double_duration=_g("double_duration", 5.0),
            triple_duration=_g("triple_duration", 5.0),
            ...
            tick_custom_file=_g("tick_custom_file", ""),
            win_custom_file=_g("win_custom_file", ""),
            profile_idx=_g("profile_idx", 0),
            ...
            spin_effects=default_spin_effect_settings(),
        )
    ...
    def from_config_entry(
        ...
        return cls(
            spin_duration=_get("spin_duration", 5.0),
            spin_preset_profile=_get("spin_preset_profile", 'z'),
            spin_preset_random=_get("spin_preset_random", False),
            spin_duration_random=_get("spin_duration_random", False),
            spin_duration_random_ratio=_get("spin_duration_random_ratio", 0.0),
            spin_phase_randomize=_get("spin_phase_randomize", 0.0),
            spin_phase_overrides=entry.get("spin_phase_overrides", None),
            spin_mode=_get("spin_mode", 0),
            double_duration=_get("double_duration", 5.0),
            triple_duration=_get("triple_duration", 5.0),
            ...
            tick_custom_file=_get("tick_custom_file", ""),
            win_custom_file=_get("win_custom_file", ""),
            profile_idx=_get("profile_idx", 0),
            ...
            spin_effects=SpinEffectSettings.from_dict(entry.get("spin_effects", {})),
        )
    ...
    def apply_to_panel(self, panel) -> None:
        ...
        sc.set_spin_profile(self.spin_preset_profile)
        sc.set_spin_preset_random(self.spin_preset_random)
        sc.set_spin_duration_random(self.spin_duration_random)
        sc.set_spin_duration_random_ratio(self.spin_duration_random_ratio)
        sc.set_spin_phase_randomize(self.spin_phase_randomize)
        sc.set_spin_phase_overrides(self.spin_phase_overrides)
        ...
        panel.set_effect_settings(self.spin_effects)
    ...
    def to_config_entry(self) -> dict:
        ...
        return {
            ...
            "spin_preset_profile":        self.spin_preset_profile,
            "spin_preset_random":         self.spin_preset_random,
            "spin_duration_random":       self.spin_duration_random,
            "spin_duration_random_ratio": self.spin_duration_random_ratio,
            "spin_phase_randomize":       self.spin_phase_randomize,
            "spin_phase_overrides":       self.spin_phase_overrides,
            ...
            "spin_effects":         self.spin_effects.to_dict(),
        }
